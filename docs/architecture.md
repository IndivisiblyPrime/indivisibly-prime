...
- **`cards/`** — `AppCard`, `BookCard`, `NftCard`, `AboutCard`, plus `ContactForm`, `MailingListForm`, `shared.tsx` (`ActionButton`, the button scale constants, and `Eyebrow` — kept but no longer rendered, see below).
...
- **No numbered eyebrows on any card** (Jack, 2026-09-10). "01 — The App" / "02 — The Book" / "03 — The NFTs" were pulled from all three cards, on both viewports, to see how they feel without them — an experiment, so the `Eyebrow` component stays in `cards/shared.tsx`, exported and unused, as the one-line way back. It used to read "03 — The NFTs", not "The Art". Tiles use `object-contain` — no cropping; they read wider because the modal is `xl`, not because the aspect was forced.
