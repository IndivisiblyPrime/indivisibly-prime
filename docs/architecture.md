...
### The desktop 1.5× scale (2026-09-09)

Jack asked for every card's supporting text and every button to be **50% larger on the web, with the phone left exactly as it was** ("the text is good across the board" on mobile). So each size is written as its old value plus an `md:` value exactly 1.5× it, and padding scales the same way:

| base | `md:` | used for |
|---|---|---|
| `text-xs` (12px) | `md:text-lg` (18) | eyebrows, section labels, date ranges, form labels, the encrypted line |
...
Buttons: `px-6 py-2.5` → `md:px-9 md:py-[0.9375rem]`, icons `h-4 w-4` → `md:h-6 md:w-6`, all inside the shared `solidButton` / `ghostButton` / `linkAction` strings — so the App/Book CTAs, Send Message and Subscribe grew together, as they're meant to. The App carousel's arrows (`h-9` → `md:h-[3.375rem]`) and dots scale too, so they don't read as leftovers under an unchanged phone.

**Card titles are the deliberate exception** — already display-sized, and 1.5× would wrap them. The About card's name went `sm:text-4xl` → `md:text-5xl` instead, matching the App/Book titles now that the body copy under it is 24px. The About social icon buttons (`h-12 w-12`, themselves a deliberate 33% bump) were left alone as well.

Every one of these is an `md:` variant, so **the phone renders exactly as it did before** — if you ever add a size here without the `md:` prefix, you've changed mobile too.
...
- **The Book title must sit on one line on the web** (Jack, 2026-09-09 — it was breaking after "The Greatest Wisdom of"). Two things hold it there: the wider `xl` modal, and `md:text-[clamp(2.25rem,3.2vw,3rem)]` on the `h2`, which shaves the type instead of wrapping below ~1300px of viewport. Measured: the default title needs 605px at 48px, and the text column is 702px at a 1512px viewport — one line holds down to ~1150px. Mobile still wraps, as it always did. A much longer `bookTitle` from Studio will wrap again; the fix then is the clamp's floor, not `whitespace-nowrap` (which would overflow the modal).
