...
- Reads `aboutAccordion[]` from Sanity but renders only `experience` and `logoFreeform` typed items (skips `contact` and `text`)
...
- **Scroll hint**: `useEffect` sets a 20-second timeout; if `window.scrollY < 480` when it fires, `showScrollHint = true`. A scroll listener clears it once scrolled past 480px. Renders as `absolute bottom-1/4 left-1/2 -translate-x-1/2` white text with `animate-flash`.
