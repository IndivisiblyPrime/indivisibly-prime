...
- **`DeskStagePhone.tsx`** — **currently mid-experiment (2026-09-02).** A thin sticky title bar (the same cover text as the desktop's `EntryCover`), then one continuous photo (`public/desk-mobile.png`) with four rough, eyeballed tap-zone rectangles (`MOBILE_TEST_SPOTS` in `data.ts`) — not calibrated outlines like the web hotspots. No more separate "scroll to reveal the desk" intro screen. This replaced the old four-stacked-scene-image approach (`PHONE_SCENES` / `desk-phone-{app,book,nft,about}.png` — data + files still present, just unused, in case Jack wants the old version back); if this photo direction sticks, re-eyeball `MOBILE_TEST_SPOTS` properly rather than trusting the current numbers.
...
- The name/socials row sits `mt-5` below the grid's top edge (nudged down from flush-with-the-photo on 2026-08-20 — "just a tad," not a measured value) and the socials are centred to the name block via the row's own `items-center`, **not** pinned to the photo's top edge. Both were tried the other way first and reverted — don't reintroduce `self-start` on the socials or you'll silently undo this.
