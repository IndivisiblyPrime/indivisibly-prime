...
| `/calibrate-mobile`, `/api/calibrate-mobile` | **local only** — same tool for the phone photo, writes `hotspots-mobile.json` |
...
- **`DeskStagePhone.tsx`** — one continuous photo (`public/desk-mobile.png`, 724×2172) that now works exactly like the web desk: true photographed outlines as hit targets, calibrated by hand in **`/calibrate-mobile`** and stored in **`hotspots-mobile.json`**, plus the App attract outline (draw-on then breathing, fades for good on the first tap of *any* object). Labels sit at their calibrated anchors. The cover title is overlaid on the photo's own top margin — **not** a nav bar, not sticky/fixed, so it scrolls away with the desk (Jack tried a sticky bar first, then asked for the overlay). No "Jack Harvey" footer, and no separate "scroll to reveal the desk" intro screen. Replaced the old four-stacked-scene-image approach (`PHONE_SCENES` / `desk-phone-*.png` — data + files still present but unused, kept as a revert path).
...
- The phone desk is **frame-less with no "Tap to open" text**; the word label is the only cue. The App is the one exception: it keeps the attract outline (draw-on, then breathing) until the first tap — matching the web desk, and cleared by tapping *any* object, not just the App.
- The phone title (`entryCoverText`) is deliberately large — `text-[2.85rem]`, tripled on 2026-09-02 from the small bar-sized text it started as. It's the page's only heading, so it carries the whole greeting.
