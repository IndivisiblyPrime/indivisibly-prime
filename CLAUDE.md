...
- **Components** live in **`src/components/desk/`**:
  ...
  - `Modal.tsx` — responsive card shell; ✕ / Esc / backdrop close; desk visible behind `bg-black/55`. Takes a `size?: "base" | "wide" | "xl"` prop (default `"base"`) controlling fluid width: `base` = `min(96vw, 46rem)`, `wide` = `min(96vw, 60rem)` (**About only** — the intentionally-different-sized card), `xl` = `min(96vw, 76rem)` (**App/Book/NFT** — these three now share one size, per Jack). Set per-card in `DeskExperience.tsx` (`size={active === "about" ? "wide" : "xl"}`).
  - `cards/` — original black/white style (the "Editorial Monograph" ivory/oxblood restyle was explored and explicitly **reverted** per Jack — don't reintroduce it without asking): `AppCard` (carousel over `appImages` rendered **inside `<PhoneFrame/>`** — see below; image column `minmax(0,340px)`, **top-aligned** (`md:items-start` on the grid, no vertical centering); Download & Website buttons — Website button only renders when `appWebsiteButtonUrl` is set in Studio, **currently empty** so it's hidden — code/schema/GROQ are all already wired, this is a Studio content task, not a code task), `BookCard` (image column matches App's `minmax(0,340px)`, also **top-aligned**; cover art `w-64 sm:w-72` with `aspect-[3/4] object-cover` — Jack's uploaded cover is a 3:2 landscape photo, so this crop **exactly doubles its rendered height** at the same width, per Jack's request; **no hardcoded "A Complete Guide" fallback anymore** — `bookSubtitle` only renders if set in Studio), `NftCard` (eyebrow reads **"03 — The NFTs"**, not "The Art"; portrait/landscape/portrait from `nftGallery`/`landscapeGallery`, tiles at `max-h-[62vh]` (~20% up from 52vh; NO cropping — proportions preserved via `object-contain`, the extra "wider" feel comes from the bigger `xl` modal, not a forced aspect change) + hover-scale + white-on-image title/year overlay, per-item URL, "All NFT Galleries" + the cryptic `EncryptedText`), `AboutCard` (**no "About Me" eyebrow above the name** — removed per Jack; socials (LinkedIn/Instagram) + ✉️ now sit **below the photo in the left column**, not next to the name; ✉️ toggles inline `ContactForm`, `cursor-pointer` explicit on all three icon buttons; Experience with company **logos in the left slot** + date as the grey line; Interests **text-only**; **mailing-list signup at the very bottom of the card** (below Other Talents & Interests, behind its own `<hr>`) — reuses `settings.comingSoonTagline` and posts to the same `/api/subscribe` endpoint as `/classic`'s Coming Soon panel, via the new `MailingListForm.tsx`), plus `ContactForm.tsx`, `MailingListForm.tsx`, and `shared.tsx` (`Eyebrow`, `ActionButton`).
  - `PhoneFrame.tsx` — reusable **iPhone-style device mockup** (pure CSS/HTML, no image asset, no Apple artwork). One fixed shell; the carousel images cross-fade *inside* its screen (`object-cover`, clipped by the concentric corner radius) — screenshots are never framed individually. Every dimension derives from one custom property `--pw` (device width) so the device scales as a unit: bezel `2.8%`, outer radius `14.5%` (≈40px at the 280px default), screen `aspect-ratio: 1170/2532`, Dynamic Island (with camera dot) + four subtle side buttons at real proportions. `--pw = min(maxWidth, 32dvh, 62vw)` — the `32dvh` term keeps the tall device inside the modal on short laptops, `62vw` keeps it clear of the modal's ✕ on phones. Props: `maxWidth` (default `17.5rem`), `className`, plus any div props (AppCard passes touch handlers for **swipe-to-change-slide**).
- **App screenshots**: six real Bonsai screens live in **`public/app-screens/*.webp`** (828×1792, ~208 KB total, converted from Jack's 1170×2532 PNGs) and are wired as `FALLBACK.appScreens` in `data.ts`. They only render when Sanity's `appImages` is empty — Studio wins. Their 1170:2532 aspect exactly matches `PhoneFrame`'s screen, so `object-cover` crops nothing; images with a different aspect (e.g. the old 941×1672 entries) get side-cropped.
