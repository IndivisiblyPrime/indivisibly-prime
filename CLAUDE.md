...
- **The App attract cue has two halves**, both gated on `revealed` (the cover having lifted — otherwise they play unseen behind it) and both fading out over 900ms on the first click. (1) Book/NFT/About are filled with `rgba(0,0,0,0.17)`, so the App is simply the brightest object — the cue works by *taking light away*, adding no chrome. (2) The App outline traces itself on once, then breathes.
