...
/**
 * Dev-only sink for the /calibrate-mobile tool: overwrites hotspots-mobile.json
 * with the corners Jack clicked. 404s in production — this writes to the source
 * tree, so it must never exist on a deployed site.
 */
export async function POST(req: Request) {
  ...
  const file = path.join(process.cwd(), "src/components/desk/hotspots-mobile.json")
  await writeFile(file, text, "utf8")
  return NextResponse.json({ ok: true, file: "src/components/desk/hotspots-mobile.json" })
}
